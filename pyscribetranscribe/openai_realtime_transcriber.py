...
class OpenAIRealtimeTranscriber:
    ...
    def __init__(self,
                 ...
                 include_options: Optional[List[str]] = None # e.g., ["item.input_audio_transcription.logprobs"]
                 ):
        """
        ...
        Args:
            ...
            on_partial_transcript (callable, optional): Callback for partial transcription results `callback(text: str)`.
        ...
        logger.info(f"OpenAIRealtimeTranscriber initialized. Model: {self.model}, Language: {self.language or 'auto'}")
    ...
    async def _get_ephemeral_token(self) -> str:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(OPENAI_REST_SESSION_URL, json=payload, headers=headers) as resp:
                    if resp.status != 200:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info(f"Attempting to connect to OpenAI WebSocket: {OPENAI_WEBSOCKET_URL}")
            connection_headers = {
            ...
            if self.on_connected:
             ...
             logger.error(error_msg) # Already logged in detail
             self.is_connected = False
             if self.on_error: self.on_error(error_msg)
             # Don't raise again, just report error
        except websockets.exceptions.InvalidStatusCode as e:
            error_msg = f"WebSocket connection failed: Status {e.status_code} - {e.response.reason if e.response else 'No reason'}."
            logger.error(error_msg, exc_info=True)
            self.is_connected = False
            if self.on_error: self.on_error(error_msg)
    ...
    async def _send_initial_config(self):
        ...
        session_config = {
             ...
             "turn_detection": {
                 ...
                 "silence_duration_ms": self.vad_settings.get("silence_duration_ms") or 1000
    ...
    async def _receive_loop(self):
        ...
        try:
            async for message in self.websocket:
                ...
                try:
                    ...
                    # It might still arrive, log it if needed for debugging, but don't rely on it.
                         ...
                         logger.debug(f"Received (but not using) transcription_session.created: {json.dumps(data)}")
                        ...
                        logger.error(f"OpenAI WebSocket Error Received: {full_error_json}")
                        # Extract details for the callback, fallback if needed
                        error_details = data.get("details", "Unknown error from OpenAI WebSocket")
                        if self.on_error:
                    ...
                    if self.on_error: self.on_error(f"Error processing message: {e}")

        except websockets.exceptions.ConnectionClosedOK:
            logger.info("WebSocket connection closed normally.")
        except websockets.exceptions.ConnectionClosedError as e:
            error_msg = f"WebSocket connection closed with error: Code {e.code}, Reason: {e.reason}"
            logger.error(error_msg)
            if self.on_error: self.on_error(error_msg)
        except Exception as e:
            ...
            await self.disconnect(from_receive_loop=True) # Ensure cleanup if loop exits unexpectedly
    ...
    async def disconnect(self, from_receive_loop=False):
        ...
        if not self.is_connected and not self.websocket and not (self.receive_task and not self.receive_task.done()):
        ...
        if self.receive_task and not self.receive_task.done() and not from_receive_loop:
            ...
            try:
                ...
                logger.debug("Receive task cancelled successfully.")
                 ...
                 logger.error(f"Error during receive task cancellation: {e}", exc_info=True)
...
async def main_test():
    ...
    try:
        ...
        for i in range(int(5 * DEFAULT_AUDIO_RATE / chunk_size)): # Simulate 5 seconds
             if not transcriber.is_connected: break # Stop if disconnected
             dummy_chunk = b'\x00' * chunk_size # Send silence
             await transcriber.send_audio_chunk(dummy_chunk)
             await asyncio.sleep(chunk_size / DEFAULT_AUDIO_RATE) # Simulate real-time interval
