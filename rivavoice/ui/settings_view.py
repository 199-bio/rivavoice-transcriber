...
from PyQt6.QtWidgets import (QWidget, QLabel, QVBoxLayout, QHBoxLayout,
                           QSpacerItem, QSizePolicy, QFileDialog, QFormLayout, QComboBox)
from PyQt6.QtCore import Qt, pyqtSignal, QSize # Added QSize
from PyQt6.QtGui import QIcon # Added QIcon

# Change to absolute imports
from rivavoice.ui.components import IconButton, StyledButton, StyledLineEdit, ToggleSwitch, KeybindSelector
...
class SettingsView(QWidget):
    ...
    def init_ui(self):
        ...
        close_icon_path = resource_path(constants.ICON_CLOSE_SVG) # Use constant & resource_path
        close_button = IconButton(close_icon_path, self, size=16) # Match SVG size in example
        ...
        model_label.setStyleSheet("font-weight: 500; margin-bottom: 2px; background-color: transparent;") # Added style
        ...
        for model in constants.AVAILABLE_MODELS:
            ...
            self.model_selector.addItem(model['display'], userData={'id': model['id'], 'provider': model['provider']})
        ...
        self.api_key_label.setStyleSheet("font-weight: 500; margin-bottom: 2px; background-color: transparent;") # Added style
        settings_layout.addWidget(self.api_key_label)

        self.api_key_input = StyledLineEdit(placeholder="Enter API key", is_password=True) # Placeholder updated dynamically
        settings_layout.addWidget(self.api_key_input)
        settings_layout.addSpacing(8) # Add spacing after item

        # --- Transcript Folder ---
        transcript_folder_label = QLabel("Transcript Folder")
        transcript_folder_label.setStyleSheet("font-weight: 500; margin-bottom: 2px; background-color: transparent;") # Added style
        ...
        self.folder_path_input.setToolTip("Transcript save location") # Tooltip stores full path
        ...
        shortcut_label.setStyleSheet("font-weight: 500; margin-bottom: 2px; background-color: transparent;") # Added style
        ...
        sound_label.setStyleSheet("font-weight: 500; background-color: transparent;") # Added style
        ...
        paste_label.setStyleSheet("font-weight: 500; background-color: transparent;") # Added style
    ...
    def _update_api_key_field(self):
        ...
        if provider == 'openai':
            ...
            api_key = self.config.get(constants.CONFIG_ELEVENLABS_API_KEY, '')
             ...
             logger.error(f"Unknown provider '{provider}' associated with model '{model_id}'")
    ...
    def load_config(self):
        ...
        index = self.model_selector.findData(lambda data: data and data.get('id') == selected_model_id)
        if index != -1:
            self.model_selector.setCurrentIndex(index)
        else:
            logger.warning(f"Saved model ID '{selected_model_id}' not found in available models. Defaulting.")
        ...
        # Common Settings
        folder_path = self.config.get(constants.CONFIG_TRANSCRIPT_FOLDER, constants.DEFAULT_TRANSCRIPT_FOLDER)
        self.folder_path_input.setText(self.format_path(folder_path)) # Update input field
        ...
        self.sounds_toggle.is_checked = self.config.get(constants.CONFIG_SOUND_EFFECTS, constants.DEFAULT_SOUND_EFFECTS) # Use correct default constant
        ...
        self.paste_toggle.is_checked = self.config.get(constants.CONFIG_PASTE_AFTER_TRANSCRIPTION, constants.DEFAULT_PASTE_AFTER_TRANSCRIPTION)
    ...
    def _update_toggle_visuals(self):
         ...
         try:
            ...
            self.sounds_toggle._toggle_position = 1.0 if self.sounds_toggle.is_checked else 0.0
            # self.nonspeech_toggle._toggle_position = 1.0 if self.nonspeech_toggle.is_checked else 0.0 # Removed
            self.paste_toggle._toggle_position = 1.0 if self.paste_toggle.is_checked else 0.0
    ...
    def browse_folder(self):
        ...
        current_folder = self.folder_path_input.toolTip() or os.path.expanduser("~") # Use input's tooltip
        ...
        if folder:
            self.folder_path_input.setText(self.format_path(folder)) # Update input field
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            constants.CONFIG_OPENAI_API_KEY: api_key if provider == 'openai' else self.config.get(constants.CONFIG_OPENAI_API_KEY, ''),
            constants.CONFIG_ELEVENLABS_API_KEY: api_key if provider == 'elevenlabs' else self.config.get(constants.CONFIG_ELEVENLABS_API_KEY, ''),
        ...
        logger.info(f"Saving settings with selected model: {selected_model_id} (Provider: {provider})")
    ...
    def show_configuration_guidance(self, show: bool, message: str = None):
        ...
        if show:
            default_message = "Welcome! Please select your desired model and enter the corresponding API key to enable recording."
