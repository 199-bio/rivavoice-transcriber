...
from PyQt6.QtCore import Qt, QTimer, QSize, QPropertyAnimation, QEasingCurve, QRect, pyqtProperty, QPoint, pyqtSignal
from PyQt6.QtGui import QPainter, QColor, QFont, QPainterPath, QBrush, QPen, QIcon
...
class PulsingOrb(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Draw pulse effect
        pulse_color = QColor(239, 68, 68, int(120 * (1 - self.pulse_size))) if self.recording else QColor(59, 130, 246, int(80 * (1 - self.pulse_size)))
...
class CardContainer(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self.drag_position = event.globalPosition().toPoint() - main_window.frameGeometry().topLeft()
            event.accept()
...
class KeybindSelector(QWidget):
    ...
    def init_ui(self):
        ...
        self.fn_key_info.setStyleSheet("""
            ...
            margin-top: 4px;
        """)
        self.fn_key_info.setVisible(False)  # Hidden by default, shown based on selection
