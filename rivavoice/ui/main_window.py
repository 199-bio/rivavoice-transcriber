...
from PyQt6.QtWidgets import (QMainWindow, QStackedWidget, QVBoxLayout,
                           QWidget, QMessageBox, QSystemTrayIcon, QMenu, QDialog, QApplication)
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, pyqtSlot, QObject # Import QObject
...
from pyscribetranscribe.transcriber import ElevenLabsTranscriber, ElevenLabsTranscriberError # Renamed
...
from pykeybindmanager import KeybindManager as PyKeybindManager, parse_keybind_string, KeybindManagerError, PermissionError as KeybindPermissionError, InvalidKeybindError, PynputImportError, play_sound_file as play_keybind_sound
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Initialize state
        self.is_processing_transcription = False  # Flag to prevent duplicate transcriptions
        ...
        if not self._is_api_key_configured():
            logger.warning("No valid API key configured for the selected model. Redirecting to Settings.")
    ...
    def check_first_run(self):
        ...
        elevenlabs_key = os.environ.get("ELEVENLABS_API_KEY") or self.config.get(constants.CONFIG_ELEVENLABS_API_KEY)
        openai_key = os.environ.get("OPENAI_API_KEY") or self.config.get(constants.CONFIG_OPENAI_API_KEY)
    ...
    def update_keybind_display(self):
        ...
        keybind_key = self.config.get(constants.CONFIG_KEYBIND, constants.DEFAULT_KEYBIND) # Use constants

        # Find the matching keybind data for display purposes
        self.keybind_data = next((k for k in SAFE_KEYBINDS if k['key'] == keybind_key), None)

        # Default to FN if not found
        if not self.keybind_data:
            # Find the default keybind data from SAFE_KEYBINDS
            self.keybind_data = next((k for k in SAFE_KEYBINDS if k['key'] == constants.DEFAULT_KEYBIND), SAFE_KEYBINDS[0]) # Fallback to first if default not found
        ...
        if hasattr(self, 'recording_view'): # Ensure view exists before updating
            ...
            keybind_key_str = self.config.get(constants.CONFIG_KEYBIND, constants.DEFAULT_KEYBIND)
            trigger_type = 'double_press_toggle' if keybind_key_str == 'fn' else 'toggle'
            self.recording_view.set_keybind_display(self.keybind_data['display'], trigger_type)
            logger.info(f"Updated keybind display to {self.keybind_data['display']}")
        else:
            logger.warning("Recording view not yet initialized during update_keybind_display.")
    ...
    def init_ui(self):
        ...
        self.setMinimumSize(458, 572) # Keep hardcoded for now, or move to constants if preferred
    ...
    def _get_current_provider_type(self):
        ...
        selected_model_id = self.config.get(constants.CONFIG_SELECTED_MODEL_ID, constants.DEFAULT_MODEL_ID)
    ...
    def setup_callbacks(self):
        ...
        logger.debug(f"Setting up recorder callbacks for provider type: {provider_type}")
        # Basic Recorder callbacks (always set)
        self.recorder.on_recording_started = self.on_recording_started
        self.recorder.on_recording_stopped = self.on_recording_stopped # Handles provider logic internally
        ...
        if not self.recorder.recording_file: # If path is None (e.g., after switching from OpenAI)
             self.recorder.recording_file = constants.DEFAULT_RECORDING_FILE # Use existing constant for full path
             logger.info(f"Ensured recorder file saving is enabled for non-OpenAI mode: {self.recorder.recording_file}")
    ...
    def _handle_final_transcript(self, text: str):
        ...
        QTimer.singleShot(0, lambda: self.on_transcription_complete(text, word_count, char_count))
    ...
    def _handle_transcription_error(self, error_message: str):
        ...
        QTimer.singleShot(0, lambda: self.recording_view.set_recording_state(False)) # Ensure UI reflects stopped state on error
        # Reset processing flag using QTimer
        QTimer.singleShot(0, lambda: setattr(self, 'is_processing_transcription', False))
    ...
    def _initialize_keybind_manager(self):
        ...
        keybind_key_str = self.config.get(constants.CONFIG_KEYBIND, constants.DEFAULT_KEYBIND)
        logger.info(f"Keybind Debug: Initializing manager for key string from config: '{keybind_key_str}'")

        try:
            self._current_keybind_obj = parse_keybind_string(keybind_key_str)
            logger.info(f"Keybind Debug: Parsed key string '{keybind_key_str}' to object: {self._current_keybind_obj}")

            # Determine trigger type based on the key
            trigger_type = 'double_press_toggle' if keybind_key_str == 'fn' else 'toggle' # Use 'toggle' for non-fn keys
            ...
            if isinstance(e, InvalidKeybindError):
                 self.show_error_message("Keybind Error", f"Invalid keybind '{keybind_key_str}' selected in config.")
        except Exception as e:
            logger.error(f"Unexpected error initializing keybind manager: {e}")
            logger.error(traceback.format_exc())
            self._current_keybind_obj = None
            self._py_keybind_manager = None
            self.show_error_message("Keybind Error", "Failed to start keyboard listener.")
    ...
    def _load_config_and_init_provider(self):
        ...
        if not model_info:
            logger.error(f"Selected model ID '{selected_model_id}' not found in AVAILABLE_MODELS. Cannot initialize provider.")
            self.show_error_message("Config Error", f"Invalid model '{selected_model_id}' selected.")
        ...
        elevenlabs_api_key = os.environ.get("ELEVENLABS_API_KEY") or self.config.get(constants.CONFIG_ELEVENLABS_API_KEY)
        openai_api_key = os.environ.get("OPENAI_API_KEY") or self.config.get(constants.CONFIG_OPENAI_API_KEY)
        ...
        try:
            ...
            if provider_type == 'elevenlabs':
                if not elevenlabs_api_key:
                    logger.error("ElevenLabs model selected, but API key is missing (check env var ELEVENLABS_API_KEY or config).")
                    self.transcriber = None
                    ...
                    self.transcriber.on_transcription_started = self.on_transcription_started
                    self.transcriber.on_transcription_complete = self.on_transcription_complete
                    self.transcriber.on_transcription_error = self._handle_transcription_error # Use shared error handler

            # Handle cases where provider is not ElevenLabs (e.g., OpenAI was selected but is disabled, or unknown)
            else:
                if provider_type == 'openai':
                     logger.error(f"OpenAI provider selected ('{selected_model_id}') but is disabled. Please choose an ElevenLabs model in settings.")
                     self.show_error_message("Provider Disabled", "OpenAI transcription is currently disabled. Please select an ElevenLabs model.")
                else: # Unknown provider
                     logger.error(f"Unknown provider type configured: {provider_type} for model '{selected_model_id}'")
                     self.show_error_message("Config Error", f"Unknown provider type: {provider_type}")
            ...
            if self.transcriber:
                logger.info(f"{provider_type} transcriber initialized successfully.")
                 ...
                 logger.warning(f"Transcriber initialization failed for provider type: {provider_type}")


        except (ValueError, ElevenLabsTranscriberError) as e: # OpenAIRealtimeError remains removed
            logger.error(f"Failed to initialize {provider_type} transcriber: {e}", exc_info=True)
            self.show_error_message("Transcriber Error", f"Failed to initialize {provider_type}: {e}")
            self.transcriber = None
        except Exception as e:
            logger.error(f"Unexpected error initializing transcriber: {e}", exc_info=True)
    ...
    def _cleanup_provider(self):
        ...
        if self.transcriber:
            provider_type = self._get_current_provider_type() # Determine type before clearing transcriber
    ...
    def _handle_keybind_activation_callback(self, press_type):
        ...
        logger.info(f"Keybind activation callback triggered. Press type: {press_type}") # Keybind Debug Log
        ...
        logger.debug(f"Keybind activation callback received (type: {press_type}, from listener thread).")
        display_key_name = self.keybind_data['display'] if self.keybind_data else 'Keybind'
        try:
            action_taken = None # Track if we start or stop
            if self.recorder.recording:
                # Stop recording on *any* press (single or double) while already recording
                logger.info(f"{display_key_name} ({press_type}) activated while recording - stopping")
                # Schedule UI update and stop_recording on the main thread
                QTimer.singleShot(0, lambda: self.update_status("Stopping recording..."))
                QTimer.singleShot(0, self.stop_recording)
                action_taken = 'stop'
            else:
                # Determine required press type based on the *current* listener config
                keybind_key_str = self.config.get(constants.CONFIG_KEYBIND, constants.DEFAULT_KEYBIND) # Get current keybind
                ...
                should_start = (required_press_type == 'single' and press_type == 'press') or \
                               (required_press_type == 'double' and press_type == 'double')

                if should_start:
                    logger.info(f"{display_key_name} ({press_type} - meets requirement {required_press_type}) activated - starting recording")
                    # Schedule UI update and start_recording on the main thread
                    QTimer.singleShot(0, lambda: self.update_status("Starting recording..."))
                    QTimer.singleShot(0, self.start_recording)
                    action_taken = 'start'
                else: # Press type didn't match requirement
                    logger.debug(f"{display_key_name} ({press_type}) activated while not recording - ignoring (requires {required_press_type}).")
    ...
    def start_recording(self):
        ...
        provider_type = self._get_current_provider_type()
        logger.info(f"Attempting to start recording with provider type: {provider_type}")
        # Sound playing moved to _handle_keybind_activation_callback

        # Ensure transcriber is ready for the selected provider
        if not self.transcriber:
             logger.error(f"Cannot start recording: Transcriber for provider '{self.provider}' not initialized.")
        ...
        # Remove OpenAIRealtimeError from exception handling
             ...
             if sys.platform == 'darwin' and isinstance(e, AudioRecorderError):
                  ...
                  self.update_status("Error: Check Mic Permissions") # More specific status
             else:
                  # Show generic error on other platforms or for other error types
                  self.show_error_message("Recording Error", f"Failed to start recording process: {e}")
             ...
             current_provider = self._get_current_provider_type()
    ...
    def stop_recording(self):
        ...
        try:
            ...
            if provider_type == 'elevenlabs':
                ...
                logger.info("ElevenLabs: Waiting for recorder thread to save file and trigger transcription.")
                # Status is updated in on_recording_stopped
            else:
                 # Use provider_type instead of self.provider
                 logger.warning(f"Stop recording called with unknown provider: {provider_type}")
                 QTimer.singleShot(0, lambda: self.recording_view.set_recording_state(False)) # Ensure UI stops
    ...
    def save_settings(self, settings):
        ...
        current_keybind = self.config.get(constants.CONFIG_KEYBIND, constants.DEFAULT_KEYBIND)
        ...
        if new_keybind != current_keybind:
             logger.info(f"Keybind changed from '{current_keybind}' to '{new_keybind}'. Re-initializing listener.")
             self._initialize_keybind_manager()
        else:
        ...
        if self._is_api_key_configured():
            ...
            logger.warning("Settings saved, but still no valid API key configured for the selected model.")
            # Optionally keep the guidance message visible or update it
            if hasattr(self.settings_view, 'show_configuration_guidance'):
                self.settings_view.show_configuration_guidance(True, "Please ensure the API key is correct for the selected model.")
    ...
    def on_recording_stopped(self, file_path: Optional[str]):
        ...
        """
        logger.info(f"Callback: Recording stopped. Provider: {self.provider}, File path: {file_path}")
        # Use QTimer for thread safety
        QTimer.singleShot(0, lambda: self.recording_view.set_recording_state(False))

        if self.provider == 'elevenlabs':
            if file_path: # Only transcribe if a file was actually saved
                QTimer.singleShot(0, lambda: self.update_status("Processing (ElevenLabs)..."))
                # Start ElevenLabs transcription in a separate thread
                threading.Thread(target=self.transcribe_recording_elevenlabs, args=(file_path,), daemon=True).start()
            else:
                logger.warning("ElevenLabs provider: Recording stopped but no file path provided (saving might be disabled or failed).")
                QTimer.singleShot(0, lambda: self.update_status("Ready")) # Reset status
            ...
            # We just update the status. Disconnect is handled by stop_recording().
            logger.info("OpenAI Realtime: Recording thread stopped. WebSocket should handle final transcript.")
            ...
            # QTimer.singleShot(0, lambda: self.update_status("Processing (OpenAI)...")) # Or keep "Finalizing"
             ...
             logger.warning(f"Recording stopped, but provider '{self.provider}' is unknown or not set.")
    ...
    def on_recording_error(self, error_message: str):
        ...
        # Use QTimer for thread safety
        QTimer.singleShot(0, lambda: self.recording_view.update_status(f"Record Error: {error_message}"))
        QTimer.singleShot(0, lambda: self.recording_view.set_recording_state(False)) # Ensure UI reflects stopped state
    ...
    def transcribe_recording_elevenlabs(self, file_path: str):
        ...
        if not isinstance(self.transcriber, ElevenLabsTranscriber):
            logger.error("Attempted ElevenLabs transcription, but transcriber is not the correct type.")
            self._handle_transcription_error("Internal Error: Invalid transcriber type.")
            return
        ...
        try:
            ...
            self.transcriber.transcribe_file(file_path, include_non_speech=include_non_speech)
        except FileNotFoundError as e:
             # This specific error is handled here as it occurs before API call
             logger.error(f"ElevenLabs transcription error: {e}")
             self._handle_transcription_error(f"Error: File not found")
        except Exception as e:
             # Catch unexpected errors during the *call* itself (not API errors handled by callback)
             logger.error(f"Unexpected error calling ElevenLabs transcribe_file: {e}", exc_info=True)
             self._handle_transcription_error(f"Error: Transcription failed")
    ...
    def on_transcription_complete(self, text: str, word_count: int, char_count: int):
        ...
        logger.debug(f"on_transcription_complete called. is_processing flag: {self.is_processing_transcription}")
        ...
        try:
            QTimer.singleShot(0, lambda: self.recording_view.update_status("Transcription complete"))
            QTimer.singleShot(0, lambda: self.recording_view.update_transcription_info(word_count, char_count))
            ...
            if self.config.get(constants.CONFIG_PASTE_AFTER_TRANSCRIPTION, True):
                if sys.platform == 'darwin':
                    ...
                    QTimer.singleShot(100, self._do_paste_applescript) # Added call with 100ms delay
                    threading.Timer(0.8, lambda: self._do_paste_applescript()).start()
                    logger.info("Paste operation scheduled.")
                else:
                    logger.info("Paste after transcription enabled, but not on macOS. Text copied only.")
    ...
    def save_transcription_to_file(self, text):
        ...
        try:
            base_folder = self.config.get(constants.CONFIG_TRANSCRIPT_FOLDER, os.path.expanduser('~/Documents'))
            transcripts_folder = os.path.join(base_folder, constants.TRANSCRIPTIONS_SUBFOLDER)
    ...
    def _simulate_keystrokes(self, text_to_type: str):
        ...
        if sys.platform != 'darwin':
            logger.warning("Keystroke simulation via AppleScript is only supported on macOS.")
            return
        ...
        '''
        try:
            logger.debug(f"Attempting to execute AppleScript for keystrokes: '{text_to_type}'") # Added log
            ...
            if process.returncode != 0:
                logger.error(f"AppleScript keystroke error (Code {process.returncode}): {stderr}")
            # else:
            ...
            logger.error(f"Error executing AppleScript for keystrokes: {e}", exc_info=True)
    ...
    def _do_paste_applescript(self):
        ...
        try:
            logger.debug("Attempting to execute AppleScript paste") # Changed log level and message
            script = 'tell application "System Events" to keystroke "v" using {command down}'
            # Use check=True and capture output for better error handling
            result = subprocess.run(['osascript', '-e', script], check=True, capture_output=True, text=True, timeout=3)
            logger.info(f"AppleScript paste executed successfully. Output: {result.stdout}")
    ...
    def quit_app(self): # Slot connected to tray manager
        ...
        if self.recorder and self.recorder.recording:
             logger.warning("Quit requested while recording - attempting to stop recording first.")
             ...
             if self.asyncio_loop and self.asyncio_loop.is_running():
                  ...
                  if future:
                       try:
                            ...
                            logger.error(f"Error waiting for OpenAI disconnect task: {e}", exc_info=True)
             else:
    ...
    def _handle_keybind_error_callback(self, error):
        ...
        logger.error(f"Keybind error callback received: {type(error).__name__} - {error}")
    ...
    def _show_permission_error_dialog(self):
        ...
        msg.setStandardButtons(QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Help)
        ...
        if result == QMessageBox.StandardButton.Help:
            try:
                if sys.platform == 'darwin': # macOS
                    ...
                    subprocess.Popen(['open', constants.MACOS_PRIVACY_URL_INPUT_MONITORING])
    ...
    def _is_api_key_configured(self) -> bool:
        ...
        try:
            ...
            if not model_info:
                logger.error(f"Cannot check API key: Model info not found for ID '{selected_model_id}'.")
            ...
            if provider_type == 'openai':
                ...
                logger.error(f"Cannot check API key: Unknown provider type '{provider_type}' for model '{selected_model_id}'.")
            ...
            if api_key:
                ...
                logger.warning(f"API key NOT found for provider '{provider_type}' (Model: {selected_model_id}). Checked env '{env_var}' and config '{key_constant}'.")
                return False
        except Exception as e:
