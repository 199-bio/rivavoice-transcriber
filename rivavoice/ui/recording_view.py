...
from rivavoice.ui.components import PulsingOrb, IconButton, StyledButton
...
class RecordingView(QWidget):
    ...
    def init_ui(self):
        ...
        logo_path = resource_path(constants.ICON_APP_SVG) # Use constant & resource_path
        try:
        ...
        settings_icon_path = resource_path(constants.ICON_SETTINGS_SVG) # Use constant & resource_path
        ...
        main_layout.addSpacerItem(QSpacerItem(20, 20, QSizePolicy.Policy.Minimum,
                                           QSizePolicy.Policy.Fixed))

        # Create orb in the middle
        orb_layout = QVBoxLayout()
        orb_layout.setContentsMargins(0, 20, 0, 0)  # Add 20px spacing at top to move orb down
        ...
        # Instructions
        self.instructions_label = QLabel(f"Double press {self.keybind_display} to record. Press again to stop")
    ...
    def set_keybind_display(self, display_name, trigger_type):
        ...
        if trigger_type == 'double_press_toggle':
            instruction_text = f"Double press {self.keybind_display} to record. Press again to stop"
        else: # 'toggle'
            instruction_text = f"Press {self.keybind_display} to record. Press again to stop"
    ...
    def update_transcription_info(self, word_count, char_count):
        ...
        self.transcription_label.setText(f"Transcription: {word_count} words, {char_count} characters")
