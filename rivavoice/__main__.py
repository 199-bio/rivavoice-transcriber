...
def setup_logging():
    ...
    log_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), f"{constants.APP_NAME.lower()}.log")
    logging.basicConfig(
    ...
    logger.info(f"Logging initialized (Level: DEBUG, File: {log_file})")
    return logger
...
def check_microphone_permission():
    ...
    try:
        ...
        import pyaudio
        p = pyaudio.PyAudio()
        stream = p.open(format=constants.AUDIO_FORMAT, # Use constant
                        channels=constants.AUDIO_CHANNELS, # Use constant
                        rate=constants.AUDIO_RATE, # Use constant
                        input=True,
                        frames_per_buffer=constants.AUDIO_CHUNK) # Use constant
...
def show_permission_instructions():
    ...
    msg.setText(f"{constants.APP_NAME} requires certain permissions to function properly:") # Use constant
    ...
    if result == QMessageBox.StandardButton.Help:
        try:
            if sys.platform == 'darwin':  # macOS
                subprocess.Popen(['open', constants.MACOS_PRIVACY_URL_GENERAL]) # Use constant
            # Add handlers for other platforms if needed
        except Exception as e:
            logging.getLogger(constants.APP_NAME).error(f"Failed to open system settings: {e}") # Use constant
...
class SingleApplication(QApplication):
    ...
    def _check_instance(self):
        ...
        if socket.waitForConnected(500):
            ...
            logging.getLogger(constants.APP_NAME).info("Another instance is already running") # Use constant
        ...
        if not self._server.listen(self.app_id):
             logger.error(f"Failed to listen on server name {self.app_id}. Another instance might be starting up or a stale server exists.")
             ...
             if socket.waitForConnected(100):
                 ...
                 logger.warning("Could not establish server. Assuming this is the first instance, but conflicts might occur.")
...
def main():
    ...
    try:
        ...
        if not app.is_first_instance:
            ...
            QMessageBox.information(None, constants.APP_NAME, f"{constants.APP_NAME} is already running")
        ...
        icon_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", constants.ICON_APP_ICNS)) # Use constant
        ...
        flag_file = os.path.expanduser(f"~/.{constants.APP_NAME.lower()}_shown_permissions") # Use constant
        ...
        if not check_microphone_permission():
            ...
            QMessageBox.warning(None, "Microphone Access Required",
        ...
        log_func = logger.critical if logger else lambda msg: print(msg, file=sys.stderr)
        ...
        try:
            ...
            if QApplication.instance():
             ...
             print(f"CRITICAL ERROR (Could not show message box: {msg_e}): {e}", file=sys.stderr)
